...
│
├── logs/                       # 애플리케이션 로그 (Logback LOG_PATH)
└── data/                       # 데이터 디렉토리 (예약)
```
...
export JAVA_OPTS="$JAVA_OPTS -DLOG_PATH=/opt/icr-solution/logs"
```

**logback-spring.xml에서 참조:**

```xml
<!-- LOG_PATH 시스템 프로퍼티 사용, 없으면 catalina.base/logs 사용 -->
...
| 로그 종류 | 위치 |
|-----------|------|
| 애플리케이션 로그 (Logback) | `$INSTALL_BASE/logs/icr-app.log` |
| 배치 로그 (Logback) | `$INSTALL_BASE/logs/icr-batch.log` |
| Tomcat 내부 로그 | `$TOMCAT_HOME/logs/catalina.out` |
| 액세스 로그 | `$TOMCAT_HOME/logs/localhost_access_log.*.txt` |

> **참고**: WAR 파일의 logback-spring.xml에서 `${LOG_PATH:-...}` 문법을 사용해야 setenv.sh의 LOG_PATH 설정이 적용됩니다.
